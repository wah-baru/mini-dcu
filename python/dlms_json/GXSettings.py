...
class GXSettings:
    ...
    def getParameters(self, args):
        parameters = GXSettings.__getParameters(
            args, "h:p:c:s:r:i:It:a:p:P:g:S:n:C:v:o:T:A:B:D:d:l:W:w:f:L:M:")
        ...
        for it in parameters:
            if it.tag == 'r':
                ...
                if it.value.startswith("0x"):
                    self.client.password = GXByteBuffer.hexToBytes(
                        it.value[2:])
                else:
                ...
                if it.value.lower() == "HDLC".lower():
                    ...
                    raise ValueError("Invalid interface type option." + it.value +
                ...
                if len(tmp) > 1:
                    ...
                    self.media.parity = Parity[tmp[2]
                                               [1: len(tmp[2]) - 1].upper()]
                ...
                try:
                    if it.value.lower() == "None".lower():
                        ...
                        raise ValueError("Invalid Ciphering option: '" + it.value +
                                         "'. (None, Authentication, Encryption, AuthenticationEncryption)")
                except Exception:
                    raise ValueError("Invalid Authentication option: '" + it.value +
                ...
                self.client.ciphering.systemTitle = GXByteBuffer.hexToBytes(
                    it.value)
            elif it.tag == 'M':
                self.client.serverSystemTitle = GXByteBuffer.hexToBytes(
                    it.value)
            elif it.tag == 'A':
                ...
                self.client.ciphering.dedicatedKey = GXByteBuffer.hexToBytes(
                    it.value)
            elif it.tag == 'o':
                ...
                if it.value.lower() == "DLMS".lower():
                    ...
                    raise ValueError("Invalid DLMS standard option: '" +
                                     it.value + "'. (DLMS, India, Italy, SaudiArabia, IDIS)")
            elif it.tag == 'c':
                ...
                self.client.serverAddress = GXDLMSClient.getServerAddressFromSerialNumber(
                    int(it.value))
            elif it.tag == 'W':
                ...
                if it.tag == 'M':
                    raise ValueError(
                        "Missing mandatory meter system title option.")
                if it.tag == 'A':
                    raise ValueError(
