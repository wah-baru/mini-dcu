    ...
    dlmsServerSettings *settings)
{
    ...
    for (pos = 0; pos != settings->base.objects.size; ++pos)
    {
        ...
        if (it->objectType == DLMS_OBJECT_TYPE_ASSOCIATION_SHORT_NAME && !settings->base.useLogicalNameReferencing)
        {
#if !defined(DLMS_IGNORE_ASSOCIATION_SHORT_NAME) && !defined(DLMS_IGNORE_MALLOC)
        ...
        else if (it->objectType == DLMS_OBJECT_TYPE_ASSOCIATION_LOGICAL_NAME && settings->base.useLogicalNameReferencing)
        {
    ...
    unsigned char invokeId)
{
    ...
    if (e->transactionStartIndex != e->transactionEndIndex || data->size > settings->base.maxPduSize
        ...
        || ((settings->base.interfaceType == DLMS_INTERFACE_TYPE_HDLC ||
            ...
            data->size > settings->base.maxInfoTX)
#endif // DLMS_IGNORE_MALLOC
    )
    {
#ifndef DLMS_IGNORE_MALLOC
    ...
    if ((ret = bb_capacity(&results, cnt)) == 0)
    {
        ...
        for (pos = 0; pos != cnt; ++pos)
        {
            ...
            case DLMS_VARIABLE_ACCESS_SPECIFICATION_VARIABLE_NAME:
                ...
                if (!settings->info.preEstablished && (settings->base.connected & DLMS_CONNECTION_STATE_DLMS) == 0 && (!e->action || e->target->shortName != 0xFA00 || e->index != 8))
                {
